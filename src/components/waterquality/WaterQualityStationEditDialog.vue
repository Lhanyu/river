<template>
  <el-dialog :model-value="visible" :title="isAdd ? '添加水质站' : '编辑水质站'" width="900px" @close="onClose" @update:model-value="onDialogUpdate">
    <el-form v-if="form" :model="form" label-width="120px" ref="formRef">
      <el-row :gutter="24">
        <el-col :span="12">
          ...
          <el-form-item label="断面名称" prop="section_name">
        ...
        <el-col :span="12">
          <el-form-item label="一级水功能区" prop="func_area1">
            <el-input v-model="form.func_area1" />
          </el-form-item>
          <el-form-item label="二级水功能区" prop="func_area2">
            <el-input v-model="form.func_area2" />
          </el-form-item>
...
async function onSave() {
  ...
  try {
    ...
    const saveData = {
      ...
      river_system_name: form.value.river_system_name || '',
      river_name: form.value.river_name || '',
      ...
      func_area1: form.value.func_area1 || '',
      func_area2: form.value.func_area2 || '',
    ...
    const response = await fetch('/api/water_quality_stations', {
      method: 'POST',
