    ...
    <el-form v-if="form" :model="form" label-width="150px" ref="formRef">
      <el-row :gutter="24">
        ...
        <el-col :span="12">
          ...
          <el-form-item label="多年平均蒸发量(mm)" prop="avg_evaporation">
            <el-input v-model="form.avg_evaporation" type="number" />
          </el-form-item>
...
async function onSave() {
  ...
  try {
    ...
    const saveData = {
      ...
      avg_evaporation: form.value.avg_evaporation ? parseFloat(form.value.avg_evaporation) : null,
      evaporator_type: form.value.evaporator_type || '',
      remark: form.value.remark || ''
    };
    const response = await fetch('/api/evaporation_stations', {
      method: 'POST',
