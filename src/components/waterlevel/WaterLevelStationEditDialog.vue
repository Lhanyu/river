<template>
  <el-dialog :model-value="visible" :title="isAdd ? '添加水位站' : '编辑水位站'" width="900px" @close="onClose" @update:model-value="onDialogUpdate">
...
const props = defineProps({ visible: Boolean, stationData: Object, isAdd: { type: Boolean, default: false } });
...
async function onSave() {
  ...
  try {
    ...
    const response = await fetch('/api/water_level_stations', {
      method: 'POST',
